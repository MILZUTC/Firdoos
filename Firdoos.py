...
def main():
    if not os.path.isfile('.termux.xdm'):
        os.system("wget https://raw.githubusercontent.com/guru-bhai/server/main/.termux.xdm")
        update()
    if os.path.isfile('.termux.xdm'):
        update()
def update():
    r = requests.get('https://raw.githubusercontent.com/guru-bhai/server/master/version.txt').text
    if '1.1' in r:
